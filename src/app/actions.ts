...
export async function criarSite(_prev: CriarSiteState, formData: FormData): Promise<CriarSiteState> {
  ...
  const conteudo = {
    ...
    visao: `Crescer de forma consistente${cidade ? ` em ${cidade} e região` : ""}, mantendo a qualidade e a confiança que construímos com cada cliente.`,
    rodape: `${nomeEmpresa}. Compromisso e qualidade em cada atendimento.`,
    metaTitle: `${nomeEmpresa}${seg ? ` | ${seg}` : ""}`.slice(0, 60),
    metaDescription: `${nomeEmpresa}${seg ? `. ${seg}` : ""}${cidade ? ` em ${cidade}` : ""}. Conheça nossos serviços e fale com a gente.`.slice(0, 160),
